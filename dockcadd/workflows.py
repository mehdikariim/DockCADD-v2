...
def assemble_matrix_results(
    ...
    for target in targets:
        result_file = output_root / target["pdb_id"] / "docking_results.txt"
        df = pd.read_csv(result_file)
        df["Compound"] = df["SMILES"].map(name_by_smiles)
        df["Target"] = target["target"]
        df["PDB ID"] = target["pdb_id"]
...
def run_matrix_from_resolution(
    ...
    for target in targets:
        ...
        perform_docking(smiles_list, target["pdb_id"], output_root=target_dir, config=run_config)
...
def run_matrix_from_config(config_path: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    report_xlsx = Path(config_data["report_xlsx"])
    report_csv = Path(config_data["report_csv"])
    dock_config = DockingRunConfig(
        ...
        binary_paths=BinaryPaths(
            ...
            p2rank_dir=Path(config_data["p2rank_dir"]) if config_data.get("p2rank_dir") else None,
        ),
    )
